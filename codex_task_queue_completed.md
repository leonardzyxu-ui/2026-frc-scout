...
- [x] Synthesize all prompt-decoder agent outputs into the prompt-decomposition artifact and queue.
...
- [x] Finish the full active/inactive teleop timeline: transition/endgame phases, FMS/Auto-derived first active side, rolling submitted shift cards, and no-loss editing after timeline changes.
...
- [x] Define a first-shift conflict authority rule so match-scoped re-confirmation converges to one authoritative match record.
- [x] Connect first-shift correction notices to a real match-scoped local notification or admin/scout queue surface.
...
- [x] After valid passphrase, collect Scout Name and 1-2 digit Scout Number, then lock both in browser cache.
- [x] Replace the normal scout home screen with a minimalist greeting and three lane panels ordered Match Scout, Pit Scout, Pre Scout.
...
- [x] At the end of the current cue, hatch a 3-agent QA review swarm where the agents cross-check the whole website plus PowerScout Mac app and each tries to find a concrete code bug, math error, or logic error before final signoff.
- [x] Fix QA finding from Noether: Match Scout V4 must not silently persist unknown first teleop shift as `Red`.
- [x] Fix QA finding from Archimedes: strategy paths must pass real contribution/defense deviations instead of deterministic zeros.
- [x] Fix QA finding from Galileo: PowerScout History/Rewards must not create a synthetic local sync ledger during passive load.
- [x] Clean Leo's task cue markdown exactly as requested.
- [x] Redesign `/strategy-preview` into a next-match dashboard with Our Win Prob, Our Margin, no Objective tile, a red/blue semicircle alliance selector, and an Explore Shifts expanded team-column view.
- [x] Make `/strategy-preview` true dashboard state: infer/guess alliance and first shift instead of exposing clickable red/blue selector controls.
- [x] Remove the redundant Our Recommended Response and Opponent Predicted Behavior summary cards from `/strategy-preview`.
