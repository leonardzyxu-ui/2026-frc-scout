...
import { PROMOTED_PPA_MODEL } from '../utils/ppaInsights';
  ...
  useEffect(() => {
    ...
    const hydrateScoutContext = async () => {
      ...
      try {
        ...
        const [username, records, preScoutCache] = await Promise.all([
  ...
  return (
    <div className="h-full overflow-y-auto bg-slate-950 px-4 py-5 text-white md:px-8">
      <div className="mx-auto flex min-h-full min-w-0 max-w-7xl flex-col gap-5 pb-8">
        <header className="flex flex-col gap-4 border-b border-slate-800 pb-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0" onClick={handleTitleClick} style={{ cursor: 'pointer', userSelect: 'none' }}>
            ...
            <p className="mt-2 max-w-2xl break-words text-sm font-semibold leading-relaxed text-slate-400">
              Pick by what is happening right now. Admin V4 turns each row into future match help, pick-list context, charts, reports, and PPA shape.
            </p>
          </div>
        ...
        {pendingTask && pendingTaskMission && (
          <section className={`admin-g2 border p-4 shadow-xl shadow-slate-950/25 ${getMissionToneClasses(pendingTaskMission.tone)}`}>
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="min-w-0">
                ...
                {pendingTask.ppa && (
                  <div className="mt-4 grid gap-3 xl:grid-cols-[minmax(0,1fr)_minmax(260px,0.42fr)]">
                    <div className="admin-g2-sm border border-white/10 bg-slate-950/35 p-3">
                      <div className="text-[10px] font-black uppercase tracking-[0.18em] opacity-70">PPA Context Waiting</div>
                      <div className="mt-3 grid gap-2 sm:grid-cols-4">
                        ...
                        <TaskMetric label="Role" value={pendingTask.ppa.role || 'Unknown'} />
                        <TaskMetric label="Risk" value={`${pendingTask.ppa.uncertainty || 'Unknown'} / ${pendingTask.ppa.tailRisk || 'Unknown'}`} />
        ...
        <main className="grid min-w-0 flex-1 gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
          <section className="min-w-0">
            ...
            <section className="admin-g2 mt-5 border border-slate-800 bg-slate-900/55 p-4 shadow-xl shadow-slate-950/25">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div className="min-w-0">
                  <div className="text-xs font-black uppercase tracking-[0.22em] text-cyan-300">Prediction Model</div>
                  <h2 className="mt-1 text-xl font-black text-white">PPA is expected value, floor, ceiling, role, and risk.</h2>
                  <p className="mt-1 text-xs font-semibold leading-relaxed text-slate-400">
                    {PROMOTED_PPA_MODEL.displayName}. {PROMOTED_PPA_MODEL.validationLine}.
                  </p>
                </div>
            ...
            <section className="admin-g2 mt-5 border border-cyan-400/20 bg-cyan-500/5 p-4 shadow-xl shadow-slate-950/25">
          ...
          <aside className="min-w-0 space-y-5">
            ...
            <section className="admin-g2 border border-slate-800 bg-slate-900/65 p-5 shadow-xl shadow-slate-950/25">
              <div className="text-xs font-black uppercase tracking-[0.22em] text-slate-500">Used By Admin V4</div>
...
function MissionButton({
  ...
  return (
    <button
      ...
      className={`admin-g2 group flex min-h-[230px] min-w-0 flex-col justify-between overflow-hidden border p-5 text-left shadow-xl shadow-slate-950/20 transition-all hover:-translate-y-0.5 active:scale-[0.99] ${getMissionToneClasses(mission.tone)} ${isPending ? 'ring-2 ring-white/30' : ''}`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
