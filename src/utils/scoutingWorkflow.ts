...
export const getAdminUseMomentRoute = (key: ScoutingUseMomentKey) => {
  switch (key) {
    case 'now':
      return '/adminv4?tab=now';
    case 'matches':
      return '/adminv4?tab=matches';
    case 'pickList':
      return '/adminv4?tab=pick-list';
    case 'visualize':
      return '/adminv4?tab=visualize';
    case 'data':
      return '/adminv4?tab=data&panel=collection';
  }
};
