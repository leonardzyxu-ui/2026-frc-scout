...
export function AdminV4PickListWorkflow({
  ...
  return (
    <div className="space-y-5">
      ...
      <AdminSurface className="border-cyan-400/25 bg-cyan-500/10 p-5">
        <FocusHeader
        ...
        <div className="mt-4 grid gap-3 xl:grid-cols-4">
              ...
              <div key={`call-sheet-${row.teamNumber}`} className="admin-g2-sm flex h-full flex-col border border-cyan-300/20 bg-slate-950/85 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <button
                      ...
                      className="mt-1 block truncate font-mono text-2xl font-black text-white hover:text-cyan-100"
                    >
                      {row.teamNumber}
                    </button>
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-black text-cyan-100">{getDecisionScore(row)}</div>
                    <div className="text-[10px] font-black uppercase text-cyan-200/70">Decision</div>
                  </div>
                </div>
                <div className="mt-4 grid gap-2 text-xs font-semibold text-slate-300">
                  <div className="admin-g2-sm border border-slate-800 bg-slate-900/70 px-3 py-2">
                    <span className="font-black text-white">Use if: </span>
                    {insight?.role.label || row.roleFit} / {row.seedFit}
                  </div>
                  <div className="admin-g2-sm border border-slate-800 bg-slate-900/70 px-3 py-2">
                  ...
                  <div className="admin-g2-sm border border-slate-800 bg-slate-900/70 px-3 py-2">
                    ...
                    {insight?.tailRisk.label || insight?.uncertainty.level || 'Need fresh scout read'}
                  </div>
                </div>
                <div className="mt-auto flex flex-wrap items-center gap-2 pt-4">
                  <AdminButton tone="cyan" onClick={() => onOpenTeam(row.teamNumber)}>
                    Open
                  </AdminButton>
          ...
          {callSheetRows.length === 0 && (
            <AdminEmptyState
              title="No live pick call sheet yet"
              why="The call sheet needs available teams after live pick status is applied."
              action="Load team evidence, clear stale statuses, or undo a mistaken pick status before alliance selection starts."
